...
def convert_to_price(x):
    """Remove dollar symbols ($) and commas (,) and convert into float."""
    if type(x)==str:
        x = x.replace('$','')
        x = x.replace(',','')
        x = float(x)
...
def convert_to_percent(x):
    """Remove percentage symbols (%) and convert into float."""
    if type(x)==str:
        x = x.replace('%','')
        x = float(x)
    return x
...
def convert_to_binary(x):
    """Replace false (f) with 0 and true (t) with 1"""
    if x == 'f':
        x = x.replace('f','0')
    elif x == 't':
        x = x.replace('t','1')
...
def count_amenities(x):
    """Calculate number of amenities within a string"""
...
def print_scores(test_accuracy, train_accuracy):
    """Print accuracy scores for Train and Test sets."""
